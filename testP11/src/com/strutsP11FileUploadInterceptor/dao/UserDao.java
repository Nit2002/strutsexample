...
public class UserDao {
	...
	public static List<Object> addUser(User user, HttpServletRequest request) {
		System.out.println(" Add Method ");
		List<Object> listObject = new ArrayList<Object>();
		int result = 0, id = 0 ;
		Connection conn = Database.getDBConnection();
		try {
			InputStream is =  user.getUserimage().getInputStream();
			byte[] imageByte = IOUtils.toByteArray(is);
			is = new ByteArrayInputStream (imageByte);
			String filePath = CustomPath.getPath(request,"user_image");
